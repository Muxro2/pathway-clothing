...
export default function ProductCard({ product, className, priority = false }: { product: Product, className?: string, priority?: boolean }) {
	return (
		<Link href={`/catalog/${product.handle}`} className={`flex-shrink-0 flex flex-col text-center ${className}`}>
			...
			<p className="mt-1 text-[12px] font-mono tracking-[0.1em] font-medium">
